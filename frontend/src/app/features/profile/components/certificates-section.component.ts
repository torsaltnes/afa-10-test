  ...
  template: `
    <div class="flex flex-col gap-4">
      ...
      <div class="flex items-center justify-between gap-4">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Professional Certificates</h3>
      ...
      @if (isAdding()) {
        <div class="rounded-xl border border-primary/30 bg-blue-50/50 p-4 dark:bg-blue-900/10">
          <p class="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300">New certificate</p>
          <app-certificate-form
            mode="create"
            [isSaving]="store.savingCertificate()"
      ...
      @if (entries().length > 0) {
          ...
          @for (entry of entries(); track entry.id) {
            <li class="rounded-xl border border-gray-200 bg-surface p-4
              ...
              @if (editingId() === entry.id) {
                <p class="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300">Edit certificate</p>
                <app-certificate-form
                  mode="edit"
                  [initialValue]="entry"
                  [isSaving]="store.savingCertificate()"
                ...
                <div class="flex items-start justify-between gap-3">
                  <div class="flex flex-col gap-0.5">
                    ...
                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ entry.issuingOrganization }}</span>
                    <span class="text-xs text-gray-400 dark:text-gray-500">Earned: {{ entry.dateEarned }}</span>
                  </div>
        ...
        <div class="flex flex-col items-center gap-3 rounded-xl border border-dashed border-gray-300 py-8
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">No certificates yet.</p>
          <button
            type="button"
            (click)="openAdd()"
...
export class CertificatesSectionComponent {
  ...
  readonly entries = input.required<CertificateEntryModel[]>();
  ...
  openEdit(entry: CertificateEntryModel): void {
    this.isAdding.set(false);
  ...
  onAdd(payload: { certificateName: string; issuingOrganization: string; dateEarned: string }): void {
    this.store.addCertificate(payload);
    this.isAdding.set(false);
  }

  onUpdate(id: string, payload: { certificateName: string; issuingOrganization: string; dateEarned: string }): void {
    this.store.updateCertificate(id, payload);
    this.editingId.set(null);
  }

  onDelete(id: string): void {
    this.store.deleteCertificate(id);
  }
}
